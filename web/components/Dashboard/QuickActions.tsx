  ...
  return (
    <div className="pointer-events-auto absolute left-1/2 bottom-6 z-20 flex -translate-x-1/2 items-center gap-6 select-none">
      {actions.map((act) => (
        ...
        >
          <div className="flex h-11 w-11 items-center justify-center rounded-full border border-cyan-500/20 bg-black/40 backdrop-blur-md shadow-lg transition-colors group-hover:border-cyan-400 group-hover:bg-cyan-500/10">
            {act.icon}
          </div>
          <span className="font-data text-[10px] font-semibold tracking-wider text-white/90 group-hover:text-cyan-300">
            {act.title}
          </span>
